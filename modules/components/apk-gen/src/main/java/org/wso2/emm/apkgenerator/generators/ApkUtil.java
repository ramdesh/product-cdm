...
public class ApkUtil {
	...
	public static String compileApk(String commonUtilPath, String serverIp, String password,
		...
		String apkPath = ApkGenerator.workingDir + Constants.FilePath.ANDROID_AGENT_APK;
		String securityTruststorePath = ApkGenerator.workingDir + Constants.FilePath.WSO2CARBON_JKS;
		String clienTruststorePath = ApkGenerator.workingDir + Constants.FilePath.CLIENT_TRUST_JKS;
		String emmSecurityTruststorePath = ApkGenerator.workingDir + Constants.FilePath.WSO2EMM_JKS;

		// .zip the above four files and create a zip file in the output folder.
		FileOperator.createZip(zipPath + zipFileName, new String[] { apkPath, securityTruststorePath,
	 ...
	 * @throws ApkGenerationException
	 */
	static void buildApk() throws ApkGenerationException {
		try {
			List<String> buildGoals = Arrays.asList(Constants.ACTION, Constants.GOAL);
			InvocationRequest request = new DefaultInvocationRequest();
			// provide POM file to invoker
			request.setPomFile(new File(ApkGenerator.workingDir + Constants.FilePath.ANDROID_AGENT +
			                            File.separator));
