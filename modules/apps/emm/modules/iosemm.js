		...
		handleProfileRequest : function(inputStream) {
			...
			try {
				...
				if (profileResponse.challengeToken != null) {
					driver.query(sqlscripts.device_pending.update4, profileResponse.udid,
				...
				var mdmConfigurations = user.getiOSMDMConfigurations(parseInt(tenantId));
				var topicId = mdmConfigurations.properties.TopicID[0];

				var certConfigurations = user.getSCEPConfiguration(parseInt(tenantId));
				...
				certificateAttributes.setCountry(new Packages.java.lang.String(country));
				certificateAttributes.setState(new Packages.java.lang.String(state));
				certificateAttributes.setLocality(new Packages.java.lang.String(locality));
				certificateAttributes.setOrganization(new Packages.java.lang.String(organization));
				certificateAttributes.setOrganizationUnit(new Packages.java.lang.String(organizationUnit));
				certificateAttributes.setEmail(new Packages.java.lang.String(email));

				var signedData = mobilityManagerService.handleProfileRequest(profileResponse.inputStream, tenantName,
                    topicId, certificateAttributes);

				return signedData;
			} catch (e) {
				log.error("Profile Request Error : " + e);
			}
			...
			try {
				...
				if (operation != null) {
					...
					if (operation.feature_code.indexOf("-") > 0) {
						...
						var payload;
						payload = common.loadPayload(new Packages.java.lang.String(operation.id),
