...
/*
    Check whether user is logged in. if the user is logged in load the tenant configuration related to the logged in user
 */
...
*/
//TODO assign session.get("emmConsoleUserLogin") to variable and check for null
if(session.get("emmConsoleUserLogin") == null && session.get("emmConsoleUserLogin") != "true" && request.getRequestURI() != appInfo().server_url + "login"){
    ...
    //TODO assign session.get("emmConsoleUser") to variable and check for null
	if(!(session.get("emmConsoleUser")['isEMMAdmin'] == true || session.get("emmConsoleUser")['isAdmin'] == true)){
	 ...
	 if(request.getParameter('user') != null && request.getParameter('user') != session.get("emmConsoleUser")['username']){
		 //TODO put this in anonymous function
...
/*
    Redirect user to the relevant page based on login
 */
index = function(){
...
context = function() {
    ...
    //based on the user type assign a role type to the admin
    if(currentUser){
